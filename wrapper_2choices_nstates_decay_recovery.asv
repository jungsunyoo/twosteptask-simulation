function data = wrapper_2choices_nstates_decay_recovery(nrits, nstates, nrtrials)
...
%         output = MBMF_stochastic_1choice_rew_nstates_sampler_decay_sim(x,rewards, states_total, nstates);
        ...
        % Parameter recovery
        results = fit_model_2choices(x, output, 'likefun_2choices_TD');
...
%     output.C(t,1) = tr_prob < tr;
        output_avgR(i) = mean(output.rewards);
