function output = MBMF_stochastic_1choice_rew_nstates_counterfactual_decay_sim(x,rew, states_total, nstates)
...
% parameters
lr = x(1);                  % learning rate for real options
kappa = x(2);                 % learning rate for counterfactual options
b = x(3);                   % softmax inverse temperature

lambda = x(4);              % eligibility trace decay
w = x(5);                   % mixing weight
gamma = x(6);
...
%         s = current_state(a); %round(double(tr_prob>tr))+2;
    ...
    Qmf(current_state_index,a) = Qmf(current_state_index,a) + lr*dtQ(1); % update TD value function
    
    % updating for counterfactual Qmf
    Qmf(current_state_index, a_cf) = Qmf(current_state_index,a_cf) + lr*kappa*(Q2(s_cf)-Qmf(current_state_index,a_cf));
    ...
    Q2(s_cf) = Q2(s_cf) + lr*kappa * (r_cf - Q2(s_cf));
    ...
    Qmf(current_state_index,a_cf) = Qmf(current_state_index,a_cf) + lambda * lr*kappa * (r_cf - Q2(s_cf));
