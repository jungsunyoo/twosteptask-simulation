function [nloglik]= likefun_sampler_highConf(params, data, flags)

% Likelihood function for memory sampling model
% This version does not sample probe trials that were not remembered

% Specify task structure
numProbes = sum(data.probet);
numTrials = length(data.rewards);
bandits   = [1 2];
numBandits = 2;
...
alpha        =  0 + (1  * (1./[1+exp(-alpha)]));
beta         =  0 + (5  * (1./[1+exp(-beta)]));            % [0 5]
beta_c       = -3 + (6  * (1./[1+exp(-beta_c)]));          % [-3 3]
    ...
    if data.probet(trialIdx) == 0 % if it's a choice trial, compute choice probability
        ...
        reward       = data.rewards(trialIdx); %get the reward that was received
            ...
            for bi = 1:numBandits
                ...
                evokedTrials = [data.probet(banditTmp)==1 & data.confidence(banditTmp) < 4]; %remembered with high confidence evoked trials
                forgottenTrials = [data.probet(banditTmp) == 1 & data.confidence(banditTmp) > 3]; % non-high confidence remembered trials
            ...
            % Now, for all combinations of rwdvals, compute choice probability
                ...
                %made on this trial
                choiceProbs(trialIdx) = sum(pmat .* (1 - 1./(1 + exp(beta_c.*congruentChoice + beta.*rvmat))));
    end
end
