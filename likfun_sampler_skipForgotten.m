function [nloglik]= likefun_sampler_skipForgotten(params, data, flags)
    ...
    if data.probet(trialIdx) == 0 % if it's a choice trial, compute choice probability
        ...
        reward       = data.rewards(trialIdx); %get the reward that was received
            ...
            for bi = 1:numBandits
                ...
                evokedTrials = [data.probet(banditTmp)==1 & data.confidence(banditTmp) < 3]; %remembered evoked trials
                forgottenTrials = [data.probet(banditTmp) == 1 & data.confidence(banditTmp) > 2]; % or forgotten evoked trials
